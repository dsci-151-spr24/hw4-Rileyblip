...
```{r}
gss16 <- gss16 %>%
  mutate(polviews = if_else(polviews%in%c("Extrmly conservative", "Conservative", "Slghtly conservative"), "Conservative", if_else(polviews%in%c("Extremely liberal", "Liberal", "Slightly liberal"), "Liberal", polviews))) 
...
***The first model performed better than the second one.***
...
```
```{r eval=FALSE}
gss16_test_rec_1 <- recipe(advfront ~ polviews + wrkstat + educ, data = gss16_test) %>%
  step_other(wrkstat, threshold = 0.10, other = "Other") %>%
  step_dummy(all_nominal(), -all_outcomes())

gss16_test_spec <- logistic_reg() %>%
  set_engine("glm")

gss16_test_wflow_1 <- workflow() %>%
  add_model(gss16_test_spec) %>%
  add_recipe(gss16_test_rec_1)

set.seed(2533)
gss16_test_folds <- vfold_cv(gss16_test, v = 5)

gss16_test_fit_rs_1 <- gss16_test_wflow_1 %>%
  fit_resamples(gss16_test_folds)

collect_metrics(gss16_test_fit_rs_1, summarize = FALSE)
collect_metrics(gss16_test_fit_rs_1)
```
```{r eval=FALSE}
gss16_test_rec_2 <- recipe(advfront ~ polviews + educ, data = gss16_test) %>%
  step_dummy(all_nominal(), -all_outcomes())

gss16_test_spec <- logistic_reg() %>%
  set_engine("glm")

gss16_test_wflow_2 <- workflow() %>%
  add_model(gss16_test_spec) %>%
  add_recipe(gss16_test_rec_2)

set.seed(2533)
gss16_test_folds <- vfold_cv(gss16_test, v = 5)

gss16_test_fit_rs_2 <- gss16_test_wflow_2 %>%
  fit_resamples(gss16_test_folds)

collect_metrics(gss16_test_fit_rs_2, summarize = FALSE)
collect_metrics(gss16_test_fit_rs_2)
```
***My answer from the previous exercise is different than the testing data because the first model for the testing data performed better than the second model.***
...
***1093 respondents said `No` and 233 said `Yes`.***
...
***Bootstrapping would take a sample of these Americans and would calculate the statistic of who has been harassed.***
...
***The lower statistic is around 0.16, and the upper statistic is around 0.20, meaning that the confidence interval is narrow.***
...
***I would expect a 90% confidence interval to be wider based on the graph in the "Bootstrapping" power point by Adam Sullivan. ***
